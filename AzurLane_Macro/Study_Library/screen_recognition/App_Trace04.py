...
# 기존의 Monitoring 기능과 App Trace가 동시에 작동한다.
# 하지만 아직까지는 전체화면이 아니면 Template이 정상적으로 작동하지 않는다.
# 또한 전체화면이 아닐 경우에는 imshow 기능조차 제대로 이루어지지 않고 있다.

# 확인해 본 결과 Template과 대조할 것이 존재하지 않으면 imshow가 작동하지 않는다.
# imshow가 이미 동작하더라도 Template과 대조할 것이 사라지면 imshow가 정지